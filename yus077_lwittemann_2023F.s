...
////////////////////////
    ...
    lda x0, merge_arr            // x0 = &merge_arr1
    lda x2, merge_arr_length     // x2 = &merge_arr1_length
    ldur x2, [x2, #0]       // x2 = merge_arr1_length
    subi x2, x2, #1         // x2 = x2 - 1     to get the last element
    lsl x2, x2, #3          // x2 = x2 * 8 <-- convert length to bytes
    add x1, x2, x0          // x1 = x2 + x0 <-- x1 = &merge_arr1[0] + length in bytes
    ...
    ADDI X2, XZR, #2       //Setting target value
    BL BinarySearch
    putint x3
    stop
...
loop:
    ...
    B.LT skip           //Skip the swap if condition is not met. arry[left] is < arry[right]
...
MergeSort:
    ...
    B store                 //Branch to store starting left index in right index
...
BinarySearch:
    ...
    SUBIS XZR, X3, #1       //Checking if number is found which means X3 == 1
    B.EQ exit               //Leave the function if number is found
    ...
    LSR X9, X0, #3         //Finding index of X0 (left) by dividing by 8
    LSR X10, X1, #3         //Finding index of X1 (right) by dividing by 8

    SUBS XZR, X1, X0        //Check if right index is equal left
    B.EQ checklast          //If it is equal, check if that last index has the value

    //Finding mid address
    SUB X11, X10, X9        //Finding right - left
    ANDI X12, X11, #1       //Finding (start + end)&1 and storing into temp register X10
    ADD X13, X11, X12       //Setting mid index to implement ciel(start + end) = (start + end) + (start + end)&1 ciel(start + end)
    LSR X13, X13, #1        //Divide the sum of indexes by 2
    LSL X13, X13, #3        //Converting mid index back to address
    
    ADD X13, X0, X13        //Finding mid = left + (right - left)/2
    LDUR X14, [X13, #0]     //Loading value of arr[mid] to register X14

    SUBS XZR, X14, X2       //Checking is arr[mid] == target (x2)
    B.NE notFound
    ADDI X3, XZR, #1        //If found set X3 to 1
    B exit

notFound:
    SUBS XZR, X14, X2       //Check if arr[mid] is less than target value
    B.GT smaller
    ADDI X0, X13, #8        //If it is not, set left address to mid + 1
    BL BinarySearch         //Call binary search with new left address
    B exit
smaller:
    SUBI X1, X13, #8        //set right address to mid - 1
    BL BinarySearch         //Call binary search with new right address
    B exit

checklast:
    LDUR X14, [X0, #0]      //Loading the last index
    SUBS XZR, X14, X2       //Checking is arr[mid] == target (x2)
    B.NE exit               //If not found, go to exit procedure
    ADDI X3, XZR, #1        //If found set X3 to 1
exit:
    LDUR FP, [SP, #8]		//load FP from stack
	LDUR LR, [SP, #16]		//load LR from stack
